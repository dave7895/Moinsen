...
std::array<int, 64 * 6> compound_table;
/*for (int i=0; i<6*64; i++){
  compound_table[i] = S(table[i], eg_table[i]);
}*/

auto temp = std::transform(table.begin(), table.end(), eg_table.begin(),
               compound_table.begin(), [](const int a, const int b) {
                 return S(static_cast<int16_t>(a), static_cast<int16_t>(b));
               });

const auto final_table = compound_table;

template <libchess::Piece piece> PhaseScore pstValue(libchess::Square const &sq) {
  return final_table[64 * piece + static_cast<int>(sq)];
}
...
PhaseScore evaluatePST(const libchess::Position &pos) {
  PhaseScore eval = 0;
